...
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=EMON-MD&fontSize=72&fontColor=fff&animation=twinkling&fontAlignY=32&desc=High%20Performance%20WhatsApp%20Bot&descAlignY=55&descSize=20" width="100%"/>
...
1. Enter your bot's WhatsApp number with country code (e.g. `8801309991724`)
...
# ── REQUIRED ─────────────────────────────────────────────────
OWNER_NUMBER=8801309991724        # No + sign
...
```bash
# After install:
nano /root/EMON-MD/.env
cd /root/EMON-MD && pm2 start dist/index.js --name emon-md
...
tmux new -s emon-md    # Start new session
npm start

# Detach:     Ctrl+B → D
# Re-attach:  tmux attach -t emon-md
# List:       tmux ls
# Kill:       tmux kill-session -t emon-md
```
...
# Keep alive with PM2
npm install -g pm2
pm2 start dist/index.js --name emon-md
pm2 save && pm2 startup
```
...
| Command | Description |
|---|---|
| `pm2 logs emon-md` | Live logs |
| `pm2 restart emon-md` | Restart |
| `pm2 stop emon-md` | Stop |
...
```env
# MongoDB
MONGO_URL=mongodb+srv://user:password@cluster.mongodb.net/emonmd

# PostgreSQL
POSTGRES_URL=postgresql://user:password@host:5432/emonmd

# MySQL
MYSQL_URL=mysql://user:password@host:3306/emonmd
...
|---|---|---|---|
| `SESSION_ID` | ✅ *one of* | — | From emon-bot-session-generator.onrender.com |
| `PAIRING_NUMBER` | ✅ *one of* | — | Phone number for terminal pairing |
| `OWNER_NUMBER` | ✅ | `8801309991724` | Your number, no `+` |
| `BOT_NAME` | ❌ | `EMON-MD` | Bot display name |
| `BOT_OWNER` | ❌ | `Emon Hawladar` | Owner display name |
| `PACKNAME` | ❌ | `EMON-MD` | Sticker pack name |
...
export default {
    ...
    async handler(sock: any, message: any, args: any[], context: any = {}) {
        const {
            ...
            channelInfo       // EMON-MD branding spread
        } = context;
