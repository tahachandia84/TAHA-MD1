...
export default {
    ...
    category: 'general',
    description: 'Get session id for EMON-MD',
    usage: '.pair 880130XXXXXX',
    async handler(sock, message, args, context) {
        ...
        try {
            const response = await axios.get(`https://emon-bot-session-generator.onrender.com/pair?number=${number}`, {
                timeout: 60000
            });
            if (response.data && response.data.code) {
                ...
                const successText = `✅ *EMON-MD PAIRING CODE*\n\n` +
                    `Code: *${pairingCode}*\n\n` +
                    `*How to use:*\n` +
