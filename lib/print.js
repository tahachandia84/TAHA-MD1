...
function printBotBanner() {
 ...
 ╔══════════════════════════════════════════════════════╗
 ║ ${chalk.hex(accentHex).bold('⚡               EMON-MD BOT SYSTEM               ⚡')} ║
 ╠══════════════════════════════════════════════════════╣
 ║ ${chalk.hex(borderHex)('👤 Owner   :')} ${chalk.hex(textHex).bold('Emon Hawladar')}                        ║
 ║ ${chalk.hex(borderHex)('📞 Number  :')} ${chalk.hex(textHex).bold('+8801309991724')}                       ║
 ║ ${chalk.hex(borderHex)('🟢 Status  :')} ${chalk.black(chalk.bgHex('#00FF66').bold(' ONLINE '))}                             ║
 ║ ${chalk.hex(borderHex)('🌍 Timezone:')} ${chalk.hex(textHex)((config.timeZone || 'Asia/Dhaka').padEnd(26, ' '))} ║
...
async function printMessage(message, sock) {
    try {
        ...
        try {
            if (fromMe) {
                senderName = sock.user?.name || 'EMON OWNER';
        ...
        if (messageText) {
            ...
            const displayText = messageText.length > maxLength
                ...
                : messageText;
            const isBotResponse = messageText.includes('EMON-MD') ||
