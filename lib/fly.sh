#!/bin/bash
# EMON-MD Fly.io One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/emonbhaiwpbot/EMON-MD/main/lib/fly.sh)
...
read -p "$(echo -e ${CYAN}App name (e.g. my-emon-md): ${NC})" APP_NAME
...
read -p "$(echo -e ${CYAN}Owner WhatsApp number (e.g. 8801309991724): ${NC})" OWNER_NUMBER
OWNER_NUMBER=${OWNER_NUMBER:-8801309991724}

read -p "$(echo -e ${CYAN}Bot name (default: EMON-MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-EMON-MD}

read -p "$(echo -e ${CYAN}MongoDB URL (recommended, press Enter to skip): ${NC})" MONGO_URL

read -p "$(echo -e ${CYAN}Timezone (default: Asia/Dhaka): ${NC})" TIMEZONE
